...
class HubConnection {
  constructor(connection, logger, protocol, reconnectPolicy) {
    ...
    this._cachedPingMessage = this._protocol.writeMessage({
      type: IHubProtocol_1.MessageType.Ping
    });
  }
  /** @internal */
  ...
  async _startInternal() {
    ...
    try {
      ...
      this._logger.log(
        ILogger_1.LogLevel.Information,
          ...
          case IHubProtocol_1.MessageType.Completion: {
            ...
            if (callback) {
              ...
              try {
                ...
                this._logger.log(
                  ILogger_1.LogLevel.Error,
                  `Stream callback threw error: ${Utils_1.getErrorString(e)}`
                );
              }
            }
            break;
  ...
  _cancelCallbacksWithError(error) {
    ...
    Object.keys(callbacks).forEach((key) => {
      ...
      try {
        ...
        this._logger.log(
          ILogger_1.LogLevel.Error,
          `Stream 'error' callback called with '${error}' threw error: ${Utils_1.getErrorString(
            e
          )}`
        );
      }
